...
public abstract class EntityRendererMixin {
	...
	@Dynamic
	@WrapOperation(method = MixinConstants.RENDER_LABEL, at = @At(value = "INVOKE", target =
		MixinConstants.GET_HEIGHT))
	...
	@Dynamic
	@WrapOperation(method = MixinConstants.POST_RENDER,
		at = @At(value = "INVOKE", target = MixinConstants.RENDER_SHADOW))
	private void pehkui$postRender$renderShadow(EntityRenderer<Entity, ?> obj, Entity entity, double x, double y,
												double z, float opacity, float tickDelta, Operation<Void> original) {
