...
public class ScaleData {
	...
	/**
	 * Returns a mutable sorted set of scale modifiers. This set already contains the default modifiers from the scale
	 * type.
	 *
	...
	public float getScale(float delta) {
		...
		final boolean canCache =
			delta == 1.0F && e != null && e.getWorld() != null && !e.getWorld().isClient &&
				(e.getType() != EntityType.PLAYER || !getScaleType().getAffectsDimensions()) &&
				!((PehkuiEntityExtensions) e).pehkui_isFirstUpdate();
	...
	/**
	 * Sets a target scale. The base scale will be gradually changed to this over the amount of ticks specified by the
	 * scale tick delay.
	 *
	...
	protected float calculateScaleForTick(int ticks, float delta) {
		final Float2FloatFunction easing =
			Optional.ofNullable(getEasing()).orElseGet(getScaleType()::getDefaultEasing);
	...
	protected int calculateRemainingScaleTicks() {
		...
		final int remaining = lastTarget == initial ? 0 :
			Math.round(getScaleTickDelay() * ((lastTarget - getBaseScale()) / (lastTarget - initial)));
	...
	public void readNbt(NbtCompound tag) {
		...
		this.baseScale = tag.contains("scale") ? tag.getFloat("scale").orElse(0F) : type.getDefaultBaseScale();
		this.prevBaseScale = tag.contains("previous") ? tag.getFloat("previous").orElse(0F) : this.baseScale;
		this.initialScale = tag.contains("initial") ? tag.getFloat("initial").orElse(0F) : this.baseScale;
		this.targetScale = tag.contains("target") ? tag.getFloat("target").orElse(0F) : this.baseScale;

		this.scaleTicks = tag.contains("ticks") ? tag.getInt("ticks").orElse(0) : 0;
		this.totalScaleTicks =
			tag.contains("total_ticks") ? tag.getInt("total_ticks").orElse(0) : type.getDefaultTickDelay();

		this.persistent = tag.contains("persistent") ? tag.getBoolean("persistent").orElse(false) : null;

		this.easing = tag.contains("easing") ?
			ScaleRegistries.getEntry(ScaleRegistries.SCALE_EASINGS,
				Identifier.tryParse(tag.getString("easing").orElse(""))) :
			null;
		...
		if (tag.contains("baseValueModifiers")) {
			final NbtElement element = tag.get("baseValueModifiers");
			if (element instanceof NbtList modifiers) {
				...
				for (int i = 0; i < modifiers.size(); i++) {
					...
					if (elementType == NbtElement.STRING_TYPE) {
						id = Identifier.tryParse(modifiers.getString(i).orElse(""));
						modifier = ScaleRegistries.getEntry(ScaleRegistries.SCALE_MODIFIERS, id);
					} else if (elementType == NbtElement.COMPOUND_TYPE) {
						final NbtCompound compound = modifiers.getCompound(i).orElse(new NbtCompound());
						id = Identifier.tryParse(compound.getString("id").orElse(""));
						modifier = ScaleRegistries.getEntry(ScaleRegistries.SCALE_MODIFIERS, id);
					} else {
						modifier = null;
					}
	...
	public NbtCompound writeNbt(NbtCompound tag) {
		...
		if (easing != null) {
			tag.put("easing",
				NbtOps.INSTANCE.createString(ScaleRegistries.getId(ScaleRegistries.SCALE_EASINGS, easing).toString()));
		}
		...
		if (!this.differingModifierCache.isEmpty()) {
			...
			for (final ScaleModifier modifier : this.differingModifierCache) {
				modifiers.add(NbtOps.INSTANCE.createString(
					ScaleRegistries.getId(ScaleRegistries.SCALE_MODIFIERS, modifier).toString()));
			}
